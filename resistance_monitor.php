...
$query  = "SELECT `cSymbol`,`resistance_value`,`grow`, `resistance_signal`,`id`,`isWatch` from stocklistIntra";
...
foreach ($row as $record) {
    ...
    if(!is_null($support_value) || !empty($support_value)) {
        ...
        if ( ($last_price >= $support_value) || ($support_signal == 1) ) {
            ...
            $query  = "UPDATE `stocklistIntra` SET `resistance_signal`='1' WHERE `cSymbol` = '$api_symbol'";
            $result = mysqli_query($GLOBALS['mysqlConnect'],$query);
