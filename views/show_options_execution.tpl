...
{foreach $datas as $value name=count}
    ...
    <tr class="show">
        ...
        <td>{$value.amount_diff}</td>
        <td>
            {$value.target} &nbsp;

            <select id="book_percentage{$smarty.foreach.count.iteration}" onchange="book_percentage('{$value.order_id}','{$smarty.foreach.count.iteration}')">
                {html_options values=$percentage_list output=$percentage_list selected={$value.book_percentage}}
            </select>
