...
#reseting the dailyentry
  $query = "UPDATE stocklistIntra SET `dailyEntry`='no', `qbuy`='0',`qvolume`='0',
...
if ($file) {
    ...
    while (($row = fgetcsv($file)) !== false) {
       ...
       $query  = "Select id from stocklistIntra where cSymbol = '$row[0]' ";
       $result = mysqli_query($GLOBALS['mysqlConnect'],$query);
    ...
    VALUES ('$sid','$open','$high','$allhigh','$low','$alllow','$close','$chng','$chng_percentage','$volume','$value',1,'$date')";
        ...
        $query = "UPDATE stocklistIntra SET dailyEntry='yes',current_volume='$volume' WHERE id = '$sid'";
        $result = mysqli_query($GLOBALS['mysqlConnect'],$query);
